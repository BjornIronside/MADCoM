...
Population::Population(Params *params, Mutator *mutator) : params(params), mutator(mutator)
{
	...
	for (int i = 0; i < 50; i++)
	{
		if (i % 2 == 0)
			...
			listeValiditeCharge.push_back(false);
		if (i % 2 == 0)
			listeValiditeTemps.push_back(true);
		else
			listeValiditeTemps.push_back(false);
	}
...
int Population::placeIndividu(SousPop *pop, Individu *indiv)
{
	...
	// Place the individual in the position sorted by solution cost
	while (i >= 0 && !placed)
	{
	...
	// Case where population is empty
	if (!placed)
	{
...
void Population::validatePen(SousPop *souspop)
{
	...
	for (int i = 0; i < souspop->nbIndiv; i++)
		souspop->individus[i]->coutSol.evaluation = souspop->individus[i]->coutSol.distance + params->penalityCapa * souspop->individus[i]->coutSol.capacityViol + params->penalityLength * souspop->individus[i]->coutSol.lengthViol;
