...
void Genetic::evolveHGA(int maxIterNonProd, int nbRec)
{
	...
	for (int i = 0; i < population->invalides->nbIndiv; i++)
		if (population->invalides->individus[i]->coutSol.isBetterFeas(bestSolFeasibility))
			bestSolFeasibility = population->invalides->individus[i]->coutSol;
	...
	while (nbIterNonProd < maxIterNonProd && (clock() - debut <= ticks) && (!params->isSearchingFeasible || population->getIndividuBestValide() == NULL))
	{
		...
		if ((rejeton->estValide && place == 0) || (rejeton->coutSol.isBetterFeas(bestSolFeasibility) && population->valides->nbIndiv == 0))
		{
			...
			if (traces && population->valides->nbIndiv == 0)
			{
				cout << "NEW BEST INFEASIBLE ";
				if (rejeton->isMutant)
					cout << "BY MUTATION ";
				else
					cout << "BY CROSSOVER ";
				cout << rejeton->coutSol.evaluation << " distance : " << rejeton->coutSol.distance << " nbRoutes : " << rejeton->coutSol.routes << " capaViol : " << rejeton->coutSol.capacityViol << " lengthViol : " << rejeton->coutSol.lengthViol << endl
					 << endl;
			}
	...
	for (int phaseGrasp = 0; phaseGrasp < nbGRASP; phaseGrasp++)
	{
		...
		for (int nbGenerationNonProd = 0; nbGenerationNonProd < nbILS && (clock() - debut <= ticks); nbGenerationNonProd++)
		{
			if (!isFirstLoop)
			{
				...
				if (parent == NULL)
					parent = population->getIndividuBestInvalide();
	...
	for (int k = 0; k < params->nbDays; k++)
	{
		...
		if (k < j2)
		{
			for (i = 0; i < (int)rejeton2->chromT[day].size(); i++)
			{
				...
				if (freqClient[ii] != 0 && params->cli[ii].jourSuiv[rejeton->chromP[ii].pat][(day - 1) % params->ancienNbDays + 1] == (int)((day - 1) % params->ancienNbDays + 1) && (rejeton->chromP[ii].dep == -1 || rejeton->chromP[ii].dep == (day - 1) / params->ancienNbDays))
				{
...
Genetic::Genetic(Params *params, Population *population, clock_t ticks, bool traces) : ticks(ticks), traces(traces), population(population), params(params)
{
