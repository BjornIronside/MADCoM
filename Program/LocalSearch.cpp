...
int LocalSearch::interRouteGeneralInsert()
{
	...
	if (!noeudU->estUnDepot)
	{
		if (shouldBeTested[1][0])
			resultMoves[1][0] = seq->evaluation(noeudU->pred->seq0_i, x->seqi_n, routeU->vehicle);
	...
	{
		if (shouldBeTested[1][0])
			resultMoves[1][0] += seq->evaluation(noeudV->pred->seq0_i, noeudU->seq1, noeudV->seqi_n, routeV->vehicle);
		if (!x->estUnDepot)
		{
			if (shouldBeTested[2][0])
				resultMoves[2][0] += seq->evaluation(noeudV->pred->seq0_i, noeudU->seq12, noeudV->seqi_n, routeV->vehicle);
			if (shouldBeTested[3][0])
				resultMoves[3][0] += seq->evaluation(noeudV->pred->seq0_i, noeudU->seq21, noeudV->seqi_n, routeV->vehicle);
		}
	}
	...
	if (!noeudV->estUnDepot)
	{
		if (!noeudU->estUnDepot)
		{
			if (shouldBeTested[1][1])
				resultMoves[1][1] += seq->evaluation(noeudV->pred->seq0_i, noeudU->seq1, y->seqi_n, routeV->vehicle);
			if (!x->estUnDepot)
			{
				if (shouldBeTested[2][1])
					resultMoves[2][1] += seq->evaluation(noeudV->pred->seq0_i, noeudU->seq12, y->seqi_n, routeV->vehicle);
				if (shouldBeTested[3][1])
					resultMoves[3][1] += seq->evaluation(noeudV->pred->seq0_i, noeudU->seq21, y->seqi_n, routeV->vehicle);
			}
		}
		if (!y->estUnDepot && !noeudU->estUnDepot && !x->estUnDepot)
		{
	...
	if (ibest == 1 || ibest == 2)
		insertNoeud(noeudU, placeV);
	if (ibest == 2)
		insertNoeud(x, noeudU);
	if (ibest == 3)
	{
		insertNoeud(x, placeV);
		insertNoeud(noeudU, x);
	}

	if (jbest == 1 || jbest == 2)
		insertNoeud(noeudV, placeU);
	if (jbest == 2)
		insertNoeud(y, noeudV);
	if (jbest == 3)
	{
		insertNoeud(y, placeU);
		insertNoeud(noeudV, y);
	}
...
int LocalSearch::intraRouteGeneralInsertDroite()
{
	...
	if (decalage >= -1 && decalage <= 1)
		return 0; // This means that U and V are already consecutive, testing these moves is useless
	...
	else if (decalage == 2)
	{
		// XUVY
		if (shouldBeTested[1][0])
			resultMoves[1][0] = seq->evaluation(Upred->seq0_i, noeudU->seq21, noeudV->seqi_n, routeU->vehicle);
		if (!noeudV->estUnDepot)
		{
			// VXUY
			if (shouldBeTested[1][1])
				resultMoves[1][1] = seq->evaluation(Upred->seq0_i, noeudV->seq1, noeudU->seq21, y->seqi_n, routeU->vehicle);
			// VUXY
			if (shouldBeTested[2][1])
				resultMoves[2][1] = seq->evaluation(Upred->seq0_i, noeudV->seq1, noeudU->seq12, y->seqi_n, routeU->vehicle);
			if (!y->estUnDepot)
			{
				// VYXU
				if (shouldBeTested[1][2])
					resultMoves[1][2] = seq->evaluation(Upred->seq0_i, noeudV->seq12, noeudU->seq21, ysuiv->seqi_n, routeU->vehicle);
				// YVXU
				if (shouldBeTested[1][3])
					resultMoves[1][3] = seq->evaluation(Upred->seq0_i, noeudV->seq21, noeudU->seq21, ysuiv->seqi_n, routeU->vehicle);
				// VYUX
				if (shouldBeTested[2][2])
					resultMoves[2][2] = seq->evaluation(Upred->seq0_i, noeudV->seq12, noeudU->seq12, ysuiv->seqi_n, routeU->vehicle);
				// YVUX
				if (shouldBeTested[2][3])
					resultMoves[2][3] = seq->evaluation(Upred->seq0_i, noeudV->seq21, noeudU->seq12, ysuiv->seqi_n, routeU->vehicle);
			}
		}
	}
	...
	else if (decalage == 2)
	{
		...
		else if (ibest == 1 && jbest == 1)
		{
			insertNoeud(x, noeudV);
			insertNoeud(noeudU, x);
		}
		else if (ibest == 1 && jbest == 2)
		{
			insertNoeud(x, y);
			insertNoeud(noeudU, x);
		}
		else if (ibest == 1 && jbest == 3)
		{
			insertNoeud(noeudV, y);
			insertNoeud(x, noeudV);
			insertNoeud(noeudU, x);
		}
		else if (ibest == 2 && jbest == 1)
		{
			insertNoeud(noeudV, noeudU->pred);
		}
		else if (ibest == 2 && jbest == 2)
		{
			insertNoeud(noeudV, noeudU->pred);
			insertNoeud(y, noeudV);
		}
		else if (ibest == 2 && jbest == 3)
		{
			insertNoeud(y, noeudU->pred);
			insertNoeud(noeudV, y);
		}
		else
			throw string("ERROR move intra-route");
	}
...
int LocalSearch::searchBetterPattern(int client)
{
	...
	for (int pat = 0; pat < (int)params->cli[client].visits.size(); pat++)
	{
		...
		if (depense < meilleureDepense - EPSILON_LS || (depense < meilleureDepense + EPSILON_LS && pattern2.dep == pattern1.dep && pattern2.pat == pattern1.pat))
		{
...
void LocalSearch::setRouteVide(int day)
{
	...
	{
		if (depots[day][route].suiv->estUnDepot)
			routeVide[day] = depots[day][route].route;
		route++;
	}
}
...
void LocalSearch::addSeqDataInPieces(Noeud *node, int length, int day)
{
	...
	while (courLenght > 0)
	{
		...
		courLenght -= temp;
		if (courLenght != 0)
			cour = clients[day][courSeq->lastNode].suiv;
	}
}
...
void LocalSearch::addReverseSeqDataInPieces(Noeud *node, int length, int day)
{
	...
	while (courLenght > 0)
	{
		...
		courLenght -= temp;
		if (courLenght != 0)
			cour = clients[day][courSeq->firstNode].suiv;
	}
