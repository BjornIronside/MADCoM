...
void Params::preleveDonnees(string nomInstance)
{
	...
	else if (type == 36) // This is the MCARP
	{
		getline(fichier, contenu);
		getline(fichier, contenu);
		fichier >> useless2;
	...
	if (type == 32)
		nbDays = 5; // PCARP instances are defined on 5 days
	else
		nbDays = 1; // Other Instances
	ancienNbDays = nbDays;
	if (type != 33)
		nbDepots = 1; // for MDCARP instances, the number of depots was already specified in commandline
	...
	if (nbVehiculesPerDep == -1)
		throw string("WARNING : some type of instances do not specify a fleet size, please specify an upper bound manually using -veh XX in the commandline");
#ifdef TURN_PENALTIES
	if (type != 34)
		throw string("When solving problem instances without turn penalties, please set the flag TURN_PENALTIES to false");
#else
	if (type == 34)
		throw string("When solving problem instances with turn penalties, please set the flag TURN_PENALTIES to true");
#endif
	...
	else if (type == 36) // MCARP
	{
		ar_parseOtherLinesMCARP();
...
void Params::getClient(int i, Client *myCli)
{
						 ...
						 // Thus we will test if the arc corresponds to an edge that has been already encountered or not
	{
		...
		else
		{
			...
			while (!isNewService) // looking for the next new service
			{
				...
				if (t_inv > t_arc)
					isNewService = true; // We found a new service
			}
	...
	else if (type == 36) // NEARP instances (we don't really read the file here but at least initialize the structures)
	{
		...
		else if (i <= ar_EdgesRequired)
			myCli->ar_nodeType = AR_CLIENT_EDGE;
		else if (i <= ar_EdgesRequired + ar_ArcsRequired)
			myCli->ar_nodeType = AR_CLIENT_ARC;
		else
			cout << "Error in parsing nbNodes" << endl;
...
void Params::ar_parseOtherLinesMCARP()
{
	// Parsing routine for MCARP
	string contenu;
	string useless;
	double myTravelCost;
	double myServiceCost;
	int iCour = 1;
	int side0;
	int side1;
	...
	for (int k = 0; k < ar_EdgesRequired; k++)
	{
		...
		fichier >> useless;
		fichier >> useless;
		fichier >> myServiceCost;
		cli[iCour].ar_serviceCost01 = myServiceCost;
		cli[iCour].ar_serviceCost10 = myServiceCost;
	...
	for (int k = 0; k < ar_ArcsRequired; k++)
	{
		...
		fichier >> myServiceCost;
		cli[iCour].ar_serviceCost01 = myServiceCost;
		cli[iCour].ar_serviceCost10 = 1.e20;
	...
	fichier >> useless;
	fichier >> side0;
	cli[0].ar_nodesExtr0 = side0;
	cli[0].ar_nodesExtr1 = side0;

	if (iCour != ar_ArcsRequired + ar_EdgesRequired + 1)
		cout << "Problem number of arcs, edges and nodes when parsing" << endl;
}
