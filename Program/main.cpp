	...
	try
	{
		...
		else
		{
			...
			while (validExist) // A feasible solution has been found, we can continue to decrease (either the number of vehicles or the distance constraint, depending on the case)
			{
				...
				if (minMaxTour && minFleetSize)
					throw string("This program was not designed to optimize jointly the fleet size and length of the maximum tour");
				...
				mutator = new Mutator(mesParametresTab[nbpop - 1]);
				populationTab.push_back(new Population(mesParametresTab[nbpop - 1], mutator));

				// Adding the individuals found in previous iterations to help the search to start
				if (nbpop >= 2)
					populationTab[nbpop - 1]->addAllIndividus(populationTab[nbpop - 2]);
				if (nbpop >= 3)
					populationTab[nbpop - 1]->addAllIndividus(populationTab[nbpop - 3]);
				// Solving
				Genetic solver(mesParametresTab[nbpop - 1], populationTab[nbpop - 1], nb_ticks_allowed, true);
				cout << "######### GA evolution ######### : "
					 << "| FLEET SIZE : " << veh << " | DIST CONSTRAINT : " << mesParametresTab[nbpop - 1]->ordreVehicules[1][0].maxRouteTime << endl;
