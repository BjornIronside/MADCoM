...
void SeqData::concatOneAfterWithPathTracking(SeqData *seq, int Vcour, Individu *myIndiv, int day)
{
	...
	if (bestCost01a < bestCost01b)
	{
		bestCost01 = bestCost01a;
		bestCostArcs[0][1] = seq->bestCostArcs[0][0];
	}
	else
	{
		bestCost01 = bestCost01b;
		bestCostArcs[0][1] = seq->bestCostArcs[0][1];
	}
	...
	if (bestCost11a < bestCost11b)
	{
		bestCost11 = bestCost11a;
		bestCostArcs[1][1] = seq->bestCostArcs[1][0];
	}
	else
	{
		bestCost11 = bestCost11b;
		bestCostArcs[1][1] = seq->bestCostArcs[1][1];
	}
	...
	if (bestCost00a < bestCost00b)
	{
		bestCost00 = bestCost00a;
		bestCostArcs[0][0] = seq->bestCostArcs[0][0];
	}
	else
	{
		bestCost00 = bestCost00b;
		bestCostArcs[0][0] = seq->bestCostArcs[0][1];
	}
	...
	if (bestCost10a < bestCost10b)
	{
		bestCost10 = bestCost10a;
		bestCostArcs[1][0] = seq->bestCostArcs[1][0];
	}
	else
	{
		bestCost10 = bestCost10b;
		bestCostArcs[1][0] = seq->bestCostArcs[1][1];
	}
...
{
	return seq1->bestCost00 + max(seq1->load - vehicle->vehicleCapacity, 0.0) * params->penalityCapa + max(seq1->bestCost00 - vehicle->maxRouteTime, 0.0) * params->penalityLength;
}
...
{
	return seq1->distance + max(seq1->load - vehicle->vehicleCapacity, 0.0) * params->penalityCapa + max(seq1->distance - vehicle->maxRouteTime, 0.0) * params->penalityLength;
}
...
{
	double totDistance = seq1->distance + seq2->distance + seq3->distance + params->timeCost[seq1->lastNode][seq2->firstNode] + params->timeCost[seq2->lastNode][seq3->firstNode];
...
{
	double totDistance = seq1->distance + seq2->distance + seq3->distance + seq4->distance + params->timeCost[seq1->lastNode][seq2->firstNode] + params->timeCost[seq2->lastNode][seq3->firstNode] + params->timeCost[seq3->lastNode][seq4->firstNode];
...
double SeqData::evaluationLB(vector<SeqData *> seqs, Vehicle *vehicle)
{
	...
	return distanceTemp + max(loadTemp - vehicle->vehicleCapacity, 0.0) * params->penalityCapa + max(distanceTemp - vehicle->maxRouteTime, 0.0) * params->penalityLength;
}
	...
	for (int i = 0; i < cli1->ar_nbModes; i++)
	{
		for (int j = 0; j < cli2->ar_nbModes; j++)
		{
			tempc = seq1->bestCost[0][i] +
					params->ar_distanceArcs[cli1->ar_Modes[i]->indexArc][cli2->ar_Modes[j]->indexArc] + seq2->bestCost[j][0];
			if (tempc < bestc)
				bestc = tempc;
		}
	}
...
double SeqData::evaluation(SeqData *seq1, SeqData *seq2, SeqData *seq3, Vehicle *vehicle)
{
	...
	for (int i = 0; i < cliFirst2->ar_nbModes; i++)
	{
		...
		for (int k = 0; k < cliLast1->ar_nbModes; k++)
		{
			...
			if (tempc < distanceTemp2[i])
				distanceTemp2[i] = tempc;
		}
	}
	...
	for (int i = 0; i < cliLast2->ar_nbModes; i++)
	{
		...
		for (int k = 0; k < cliFirst2->ar_nbModes; k++)
		{
			...
			if (tempc < distanceTemp[i])
				distanceTemp[i] = tempc;
		}
	}
	...
	for (int i = 0; i < cliLast2->ar_nbModes; i++)
	{
		...
		{
			tempc = distanceTemp[i] + params->ar_distanceArcs[cliLast2->ar_Modes[i]->indexArc][cliFirst3->ar_Modes[j]->indexArc] + seq3->bestCost[j][0];
			if (tempc < bestc)
				bestc = tempc;
		}
	}
...
double SeqData::evaluationLB(SeqData *seq1, SeqData *seq2, Vehicle *vehicle)
{
	return seq1->distance + seq2->distance + params->timeCost[seq1->lastNode][seq2->firstNode] + max(seq1->load + seq2->load - vehicle->vehicleCapacity, 0.0) * params->penalityCapa;
}

double SeqData::evaluationLB(SeqData *seq1, SeqData *seq2, SeqData *seq3, Vehicle *vehicle)
{
	return seq1->distance + seq2->distance + seq3->distance + params->timeCost[seq1->lastNode][seq2->firstNode] + params->timeCost[seq2->lastNode][seq3->firstNode] + max(seq1->load + seq2->load + seq3->load - vehicle->vehicleCapacity, 0.0) * params->penalityCapa;
}

double SeqData::evaluationLB(SeqData *seq1, SeqData *seq2, SeqData *seq3, SeqData *seq4, Vehicle *vehicle)
{
	return seq1->distance + seq2->distance + seq3->distance + seq4->distance + params->timeCost[seq1->lastNode][seq2->firstNode] + params->timeCost[seq2->lastNode][seq3->firstNode] + params->timeCost[seq3->lastNode][seq4->firstNode] + max(seq1->load + seq2->load + seq3->load + seq4->load - vehicle->vehicleCapacity, 0.0) * params->penalityCapa;
}
