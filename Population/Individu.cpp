		...
		for (int i = params->nbDepots; i < params->nbClients + params->nbDepots; i++)
		{
			if (params->cli[i].freq != 0)
			{
				...
				for (int k = 0; k < params->ancienNbDays; k++)
				{
					...
					if (temp2 == 1)
						chromT[params->ancienNbDays - k + depot * params->ancienNbDays].push_back(i);
				}
			}
		}
...
void Individu::generalSplit()
{
	...
	for (int k = 1; k <= params->nbDays; k++)
		if (chromT[k].size() != 0 && splitSimple(k) == 0)
			splitLF(k); // if its not successful we run the Split with limited fleet
...
int Individu::splitSimple(int k)
{
	...
	if (j == 0)
	{
		...
		// cout << coutSol.capacityViol << ' ' << coutSol.evaluation << ' ' << coutSol.distance << ' ' << coutSol.routes;
...
void Individu::testPatternCorrectness()
{
	...
	for (int i = params->nbDepots; i < params->nbClients + params->nbDepots; i++)
		if (frequencies[i] != params->cli[i].freq)
			throw string("Incorrect solution with missing visits");

	for (int i = params->nbDepots; i < params->nbClients + params->nbDepots; i++)
		if (params->cli[i].visitsDyn[chromP[i].pat] == -1)
			throw string("Incorrect solution with missing visits");

	for (int i = params->nbDepots; i < params->nbClients + params->nbDepots; i++)
		if (chromP[i].dep < 0 || chromP[i].dep >= params->nbDepots)
			throw string("Incorrect solution with missing visits");
}
...
void Individu::printRoutes()
{
	int begin;
	int end;
	int nVehicles{params->nombreVehicules[1]};
	for (int i{nVehicles - nbRoutes}; i < nVehicles; i++)
	{
		begin = chromR[1][i];
		if (i == nVehicles - 1)
			end = chromT[1].size();
